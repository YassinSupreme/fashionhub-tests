...
When(
  ...
  async function (this: FashionHubWorld) {
...
Then(
  'the navigation bar contains a link to {string}',
  async function (this: FashionHubWorld, linkText: string) {
    ...
    expect(found, `Nav links: [${links.join(', ')}] — expected to include "${linkText}"`).toBe(
      true,
    );
  },
);
...
Then(
  'the feature section shows at least {int} highlight items',
...
Then(
  'all product prices match the currency format "$X.XX"',
  async function (this: FashionHubWorld) {
    const names = await this.productsPage.getProductNames();
...
Then(
  'the price of product {string} matches the currency format',
  async function (this: FashionHubWorld, productName: string) {
    const price = await this.productsPage.getProductPrice(productName);
    expect(price).toMatch(/^\$\d+\.\d{2}$/);
  },
);

Then(
  'the about page contains the subsection {string}',
  async function (this: FashionHubWorld, sectionName: string) {
    const headings = await this.aboutPage.getSectionHeadings();
    const found = headings.some((h: string) => h.toLowerCase().includes(sectionName.toLowerCase()));
    expect(found, `Section "${sectionName}" not found in: [${headings.join(', ')}]`).toBe(true);
  },
);
